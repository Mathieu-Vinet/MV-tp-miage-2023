...
```
Dans TodoItem.java et la RG1 est dans todoListController.java
```
*Cette règle est-elle facilement testable par un test unitaire ?*
```
Si on peut modifier la valeur Time d'un objet alors on peut facilement la tester en TU, par contre si on ne peut pas cela semble impossible ou alors TU d'une durée de + de 24H.
```
...
```
docker run --> lancer une commande docker
-it --> POur lancer en tâche de fond
-e POSTGRES_PASSWORD=password --> Pour créer la variable d'environement POSTGRES_PASSWORD
-p 5432:5432--> Donne le port
postgres --> type de base
```
* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
```
Car il n'est pas inject au démarrage.
```
...
```
Fonctionnel vérifier avec PostMan
```
...
```
Isoler la logique métier des services techniques d'une application.
Faciliter la mise en place de test.
```
...
```
AssertEquals, et on vérifie si .finalContent() contient LATE
```
